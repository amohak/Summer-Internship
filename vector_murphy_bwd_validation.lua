...
for i=1,opt.maxepoch - 1 do
	filename[i] = string.format("/home/arjun/mohak/lstm/murphy_vector_bwd_%d.t7",i)
end
...
for th=1,9 do
	...
	filen = string.format("/home/arjun/mohak/lstm/murphy_vector_bwd_%d.t7",epoch)
	print("Threshold "..th)
	...
	while iteration < validBatches do
		...
		for key,value in ipairs(outputs) do
			exp_value = torch.exp(value)
			if targets[key][1] == 2 then
				if exp_value[1][2] > thresholds[th] then
					tp = tp + 1
					pk[#pk + 1] = length - key + 1
				else
					fn = fn + 1
					npk[#npk + 1] = length - key + 1
				end
			else
				if exp_value[1][2] > thresholds[th] then
					fp = fp + 1
					pnk[#pnk + 1] = length - key + 1
				else
					tn = tn + 1
					npnk[#npnk + 1] = length - key + 1
				end
			end
		end	
		print(iteration)
	...
	print("Not predicted but keyword : ",fn)
